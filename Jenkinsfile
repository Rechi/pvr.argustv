...
def buildPlugin(Map params = [:])
{
	...
	for (int i = 0; i < platforms.size(); ++i)
	{
		...
		tasks[platform] = {
			node(platform)
			{
				lock("${env.NODE_NAME}/binary-addons/${platform}-${version}")
				{
					ws("workspace/binary-addons/kodi-${platform}-${version}")
					{
						stage("prepare (${platform})")
						{
							...
							checkout([
								changelog: false,
								$class: 'GitSCM',
								branches: [[name: "*/feature/binaryAddonsJenkins"]],
								doGenerateSubmoduleConfigurations: false,
								extensions: [[$class: 'CloneOption', 'honorRefspec': true, 'noTags': true, 'reference': "${pwd}/../../kodi"]],
								userRemoteConfigs: [[refspec: "+refs/heads/feature/binaryAddonsJenkins:refs/remotes/origin/feature/binaryAddonsJenkins", url: 'https://github.com/Rechi/xbmc.git']]
							])
